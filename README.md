...
* `__init__(self, keys, data)`: The `Row()` initializer. It will initalize all the internal data structure of the `Row()` object. The `key` argument should be an iterable of the keys in the table in which the row will reside in. All the items in the `key` argument should be `str`. The `data` argument is also an iterable that contains all the data in the given row. `key` and `data` should be of the same length and the i-th item in `data` should be the corresponding data to the i-th item in `key`.
...
* `keys(self), get_id(self)`: Return the keys in the row as a tuple or the ID field’s value of the row respectively.

* `__getitem__(self, key)`, `__setitem__(self, key, value)`: the data fetch and set methods. When called, they will return the item corresponding to the `key` or set the item corresponding to the `key` with `value` respectively.

* `__iter__(self)`, `__next__(self)`: Implement an iterator on the row. Everytime the iterator is iterated, it will return a column in the table, in the lexicographically ascending order
...
As we mentioned above, a table usually consists of multiple rows. Those rows in a table must have the same set of columns. In this task, you are going to build your `Table()` object to formaize those observations.

The `Table()` object should read in the table from a CSV file and divide the table into rows so that you can do some operations in the following tasks. The CSV file usually contains multiple rows. Every row has multiple columns of fields, which are separated by a single comma (`,`) and zero or more spaces before or after it, and ends with a `\n` in Python. You should ignore the empty lines in the file. The first row of the CSV file is the header, which gives the corresponding keys to each of the columns. An example of a CSV file is given in the next part. The files given to you in test cases are all encoded with `utf-8` and they do not have any empty column (empty columns occur when two commas have no data or have pure spaces between them. e.g.,  `,,`).
...
* `__init__(self, filename, rows=None, keys=None)`: Initialize your `Table()` object in this method. 
  ...
  Argument `filename` will be the relative path to your current working directory of the CSV file you should read in. The file will always be available in the file system.  You are allowed to design your own internal structure of this object as long as your attributes are not accessible from outside.

  Your `__init__()` should support two approaches to construct the `Table()` object. The first one is from a file. This approach should happen when the third argument `rows` is set to `None`. You should read in the table from the CSV file as specified in `filename`. The file that you will read in is always guaranteed to exist in your file system. The second one is from multiple rows. This should happen when `rows` is set to an iterable that contains a number of instances of the `Row()` object and `keys` is set to a non-empty list of keys in your table. In this case, you should construct your `Table()` object from those rows and you are not supposed to read the file as specified by `filename`. Those two argument should be ignored if only one of them is presented. 

  As you may have noticed, the data fields you read in from the CSV file are all strings. It is not convenient to use strings to perform some operations in the following tasks. So you should convert all fields into an integer except for `AIRLINE`, `TAIL_NUMBER`, `ORIGIN_AIRPORT`, `DESTINATION_AIRPORT` fields. You are not excepted to do the conversion when constructing from multiple `Row()` objects and you may notice we do same transformation in class `Row`;
  
* `keys(self)`: The same as `Row()`. Return the keys in the `Table()` as a list in lexicographically ascending order;

* `get_table_name()`: Return the table name of the table. The table's name is the `filename` argument in `__init__()`;

* `__getitem__(self, id)`: Index the table by the id. This method should return the row, whose the `ID` field that corresponds to `id`, as a `Row()` object. Raise `ValueError` if the table does not contain such a row. You are not expected to return a copy of the row since we want any modification on the row being reflected in the table (think about why);

* `__iter__(self)`, `__next__(self)`: Implement an iterator on the table. Your iterator should return rows as `Row()` objects in lexicographically ascending order of the `ID` field. Your iterator is going to be called multiple times;

* `__len__(self)`: Return the number of rows in the table;

* Feel free to define other methods or helper functions (classes) you like.

Once you finish your implementation , you can test your implementation as below:
...
Now you need to implement the class `Query` which is used to filter data from a CSV file based on conditions given by the user.
...
-  `condition` contains a list of dictionaries. Each dictionary contains three key-value pairs, `key`, `value` and `operator`. 
  ...
  Each dictionary gives you a *condition*. A condition is telling a way to filter the row by the field `key`. A condition is statisfied for a row if and only if the field corresponding to the `key` satisfy the binary relationship specified by the binary `operator` with `value`. Or as pseudo code: `(row[key] operator value) == True` (e.g., `(row[‘YEAR’] == '2015’) == True`).
  
  Include all the rows that satisfy all the conditions in `condition` in the result of the query. If the `condition` list is empty, all the rows in the table should be included. The data type of `key` and `value` is same as in HW 1. That is that the corresponding value of `AIRLINE`, `TAIL_NUMBER`, `ORIGIN_AIRPORT`, `DESTINATION_AIRPORT` should be treated as string and other columns should be treated as integer;
  
- The `filename` tells the file from which you should read in the data set for you query.

Your implementation of the query should be in the class `Query()`.

* `__init__(self, query)`: The initalizer of the `Query()` class. `query` is the query you need to do, in the format we described above;
...
This query tells your program to find all the data rows that is before 2015 and the airplane’s tail number (TAIL_NUMBER, 国籍注册号) is not `N97W2`.

### Task 3 Data Exportation

Sometimes, you may want to shutdown your computer so exporting the data from a table to a file on disk for further reference is a necessary. Of course, you can do this manually everytime you want to do so. But it will be a great thing for you to provide an interface to accomplish this task that could export selected columns of the table to a file.
...
* `Table.export(columns,filename=None)`: The method takes in two arguments, `columns` and `filename`;
  ...
  * `filename` gives you the location where the table should be exported to. If the file exists at the time, overwrite the file. If it does not exist, create it beforehand. If this argument is  `None`, overwrite the file where the table reads in from;
...
You are going to implement data aggregation for your query in this task. This task is a bonus task. Finishing this task will give you an additional 10% in your final score of this assignment.

Sometimes you are going to query some quantity of some groups of rows in the table. For example, your user may want to know about the average distance of flights of airlines like American Airlines (AA), Spirit Airlines (NK) and Air China (CA). Of course he/she could filter out the flights' distance from all three airlines and use a Python program to calculate the average distance of flights. But if your program could help him/her to calculate the result with a single query, it will be great.

The operation we described above is called data aggregation formally. In this task, you are implementing a new class called `AggQuery()` which is inherited from `Query()` which allows some aggregation operations on tables. Since it is inherited from `Query()`, `AggQuery()` should provide the same interface (methods) as `Query()`.
...
* `function` is what you are required to operate on the `column` field. In this task, you need to support only two functions: 

  * `AVG`: take the average value of all the fields of the column;

  *  `MAX`: get the maximum value along all the fields of the column; 

  The `AVG` function will only be applied to the columns that support both arithmetic addition (`+`) and arithmetic division (`/`), which are all the columns **except** `AIRLINE`, `TAIL_NUMBER`, `ORIGIN_AIRPORT`, `DESTINATION_AIRPORT`. The `MAX` function will be applied to the all columns;

* The `group_by` gives a column name as string. you need to group the rows with the same value in the column given by `group_by` together and then use the aggregation functions (`AVG` or `MAX`) to calculate `column`'s' value of the groups. 

More specifically, the aggregation works the following way: First, you need to filter the data with the `condition` the same way as in task 1. Then group up the rows that is included in the result of last step by the `group_by` field and calculate the required value of the `column` field  with `function`. The result table should name the column as `${function}(${column}) ` (e.g., `MAX(DISTANCE)`). 

The rows in the result table need a ID field since our `Table()` class uses it to distinguish different rows. However, there is no reasonable ID for the rows here after data aggregation, so you may (and should) assign an arbitrary unique ID for every row in the table. In other words, in your implementation, rows can be in any order.
